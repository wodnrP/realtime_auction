...
class Penalty(models.Model):
    ...
    def update_user_access(sender, instance, created, **kwargs):
        if created:
            ...
            buy_penalty = Penalty.objects.filter(user_id=instance.user_id.id, penalty_type='buy').count()
            sell_penalty = Penalty.objects.filter(user_id=instance.user_id.id, penalty_type='sell').count()
